...
<p>
	This basic test about the use of the C++ programming language exploits the <code><i>'std::numeric_limits'</i></code> template class (defined in the <code><i>'< limits >'</i></code> header file) for analyzing several properties of the <code><i>'char'</i></code> C++ built-in type. As you know, this type is capable of holding the characters representation, which can be most efficiently processed on the target system. Usually, any <code>8</code>-bit character of the <A href="https://en.wikipedia.org/wiki/ASCII"><i>ASCII</i></A> standard is representable by this type. Broadly speaking, in this test, we exploit the <code><i>'std::numeric_limits< char >'</i></code> specialized class, evaluating and validating even the new capabilities of this class, introduced by the C++11 standard.
</p>
<p>
	<!--- C++11 Alert --->
