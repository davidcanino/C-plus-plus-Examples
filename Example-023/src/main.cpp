/**********************************************************************************************************************************************************************************
 * Examples in the C++ language (the 'Example-023' Test).
 *
...
int main(void)
{
    /* This is the 'Example-023' Test, where the properties of the 'char32_t' C++ built-in type are analyzed. */
    cout << endl << "\tThis is the \'Example-023\' Test in the C++ language." << endl;
    cout << "\t=======================================================================================================================================================" << endl << endl;
    cout.flush();
    
    /* TASK #1 - retrieving several properties of the 'char32_t' C++ built-in type! */
    cout << "\tThe \'char32_t\' C++ built-in type:" << endl << endl;
    ...
    if(std::numeric_limits<char32_t>::has_infinity) cout << "\t-) represents the \'infinity\' value as " << std::numeric_limits<char32_t>::infinity() << ";" << endl << endl;
    ...
    if(std::numeric_limits<char32_t>::has_denorm) cout << "\t-) represents subnorm values;" << endl;
    ...
    cout << "\t-) has " << (char32_t)std::numeric_limits<char32_t>::epsilon() << " as the machine epsilon, i.e., the difference between 1.0 and the next value, representable as a \'char32_t\' value;" << endl;
    cout << "\t-) has " << (char32_t)std::numeric_limits<char32_t>::round_error() << " as the maximum rounding error;" << endl;
    cout << "\t-) has " << (char32_t)std::numeric_limits<char32_t>::denorm_min() << " as the minimum positive subnormal \'char32_t\' value;" << endl;
    ...
    switch (std::numeric_limits<char32_t>::round_style)
    {
        case round_toward_zero:
            ...
            cout << "\t-) exploits the \'round toward zero\' policy for rounding not completely \'char32_t\' values." << endl << endl;
            cout.flush();
            break;
        ...
        case round_toward_neg_infinity:
            ...
            cout << "\t-) exploits the \'round to the negative infinity\' policy for rounding not completely \'char32_t\' values." << endl << endl;
            cout.flush();
            break;
            
        default:
            
            cout << "\t-) does not exploit a policy for rounding not completely \'char32_t\' values." << endl << endl;
            cout.flush();
            break;
    }
