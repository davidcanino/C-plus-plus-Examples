...
<H3>The 'Example-022' Test</H3>

<p>
	This basic test about the use of the C++ programming language exploits the <code><i>'std::numeric_limits'</i></code> template class (defined in the <code><i>'< limits >'</i></code> header file) for analyzing several properties of the <code><i>'char16_t'</i></code> C++ built-in type. This type is used for the <i>UTF-16</i> characters representation, required to be large enough to represent any Unicode code unit using 16 bits. Broadly speaking, we exploit the <code><i>'std::numeric_limits< char16_t >'</i></code> specialized class. Also the new capabilities of this class, introduced by the C++11 standard, are evaluated and exploited.
</p>
<p>In particular, this test consists of the <i>'Example-022'</i> executable file, starting uniquely from the <i>'main.cpp'</i> C++ source file (in the <i><code>'src'</code></i> folder) without using other dependencies.</p>
...
<p>
	As mentioned above, the <i>'Example-022'</i> executable file is created. For instance, this test is executed by running:
</p>
<pre>./Example-022</pre>
<p>
