 ...
 *
 * main.cpp - the C++ source file, implementing the main function for the 'Example-016' Test.
 **********************************************************************************************************************************************************************************/
...
int main(void)
{
    /* This is the 'Example-016' Test, where the properties of the 'short int' C++ built-in type are analyzed. */
    cout << endl << "\tThis is the \'Example-016\' Test in the C++ language." << endl;
    cout << "\t=======================================================================================================================================================" << endl << endl;
    cout.flush();
    
    /* TASK #1 - retrieving several properties of the 'short int' C++ built-in type! */
    cout << "\tThe \'short int\' C++ built-in type:" << endl << endl;
    ...
    if(std::numeric_limits<short int>::has_infinity) cout << "\t-) represents the \'infinity\' value as " << std::numeric_limits<short int>::infinity() << ";" << endl << endl;
    else cout << "\t-) does not represent the \'infinity\' value;" << endl << endl;
    cout.flush();
    
    /* TASK #2 - continuing our analysis! */
    if(std::numeric_limits<short int>::is_iec559) cout << "\t-) fulfills the requirements of the IEC 559 (IEEE 754) standard;" << endl;
    ...
    if(std::numeric_limits<short int>::has_denorm) cout << "\t-) represents subnorm values;" << endl;
    ...
    cout << "\t-) has " << (short int)std::numeric_limits<short int>::epsilon() << " as the machine epsilon, i.e., the difference between 1.0 and the next value, representable as a \'short int\' value;" << endl;
    cout << "\t-) has " << (short int)std::numeric_limits<short int>::round_error() << " as the maximum rounding error;" << endl;
    cout << "\t-) has " << (short int)std::numeric_limits<short int>::denorm_min() << " as the minimum positive subnormal \'short int\' value;" << endl;
    ...
    switch (std::numeric_limits<short int>::round_style)
    {
        case round_toward_zero:
            ...
            cout << "\t-) exploits the \'round toward zero\' policy for rounding not completely \'short int\' values." << endl << endl;
            cout.flush();
            break;
            
        case round_to_nearest:
            
            cout << "\t-) exploits the \'round to nearest\' policy for rounding not completely \'short int\' values." << endl << endl;
            cout.flush();
            break;

        case round_toward_infinity:
            
            cout << "\t-) exploits the \'round to the positive infinity\' policy for rounding not completely \'short int\' values." << endl << endl;
            cout.flush();
            break;
            
        case round_toward_neg_infinity:
            
            cout << "\t-) exploits the \'round to the negative infinity\' policy for rounding not completely \'short int\' values." << endl << endl;
            cout.flush();
            break;
            
        default:
            
            cout << "\t-) does not exploit a policy for rounding not completely \'short int\' values." << endl << endl;
            cout.flush();
            break;
    }
    ...
    cout << "\t-) the minimum value for an object of \'short int\' type is " << SHRT_MIN << " (see the SHRT_MIN macro);" << endl;
    cout << "\t-) the maximum value for an object of \'short int\' type is " << SHRT_MAX << " (see the SHRT_MAX macro)." << endl << endl;
