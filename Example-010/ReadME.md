...
<p>
	This test about the use of the C++ programming language shows how exploiting several <i>'variadic'</i> functions, defined and implemented on a separate C++ header and source file, respectively, in a C++ program. By construction, any <i>'variadic'</i> function has indefinite arity, i.e., accepts a variable number of arguments. The real number for the arguments of a <i>'variadic'</i> function is known only when it is actually invoked. Broadly speaking, this test is another example of the <i>separate (modular) compiling</i> techniques for exploiting a (third-party) module in a C++ program.
</p>
<p><hr></p>
...
<ul>
	...
	<li>
		the <i>'variadic'</i> functions of interest are defined in the <i>'myvariadic.h'</i> C++ header file (in the <i><code>'include'</code></i> folder);
	</li>
	<li>
		the <i>'variadic'</i> functions of interest are implemented in the <i>'myvariadic.cpp'</i> C++ source file (in the <i><code>'src'</code></i> folder).
	</li>
</ul>
</p>
...
<p>
	As mentioned above, the <i>'Example-010'</i> executable file is created. For instance, this test is executed by running:
</p>
<pre>./Example-010</pre>
<p>
