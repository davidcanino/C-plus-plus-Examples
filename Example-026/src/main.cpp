/**********************************************************************************************************************************************************************************
 * Examples in the C++ language (the 'Example-026' Test).
 *
 ...
 *
 * main.cpp - the C++ source file, implementing the main function for the 'Example-026' Test.
 **********************************************************************************************************************************************************************************/
...
/// The main function for the <i>'Example-026'</i> Test.
int main(void)
{
    /* This is the 'Example-026' Test, where the properties of the 'unsigned long long int' C++ built-in type are analyzed. */
    cout << endl << "\tThis is the \'Example-026\' Test in the C++ language." << endl;
    cout << "\t=======================================================================================================================================================" << endl << endl;
    cout.flush();
    
    /* TASK #1 - retrieving several properties of the 'unsigned long long int' C++ built-in type! */
    cout << "\tThe \'unsigned long long int\' C++ built-in type:" << endl << endl;
    ...
    cout << "\t-) requires " << sizeof(unsigned long long int) << " bytes for being represented;" << endl;
    ...
    if(std::numeric_limits<unsigned long long int>::has_infinity) cout << "\t-) represents the \'infinity\' value as " << std::numeric_limits<unsigned long long int>::infinity() << ";" << endl << endl;
    else cout << "\t-) does not represent the \'infinity\' value;" << endl << endl;
    cout.flush();
    
    /* TASK #2 - continuing our analysis! */
    if(std::numeric_limits<unsigned long long int>::is_iec559) cout << "\t-) fulfills the requirements of the IEC 559 (IEEE 754) standard;" << endl;
    ...
    if(std::numeric_limits<unsigned long long int>::has_denorm) cout << "\t-) represents subnorm values;" << endl;
    ...
    cout << "\t-) has " << (unsigned long long int)std::numeric_limits<unsigned long long int>::epsilon() << " as the machine epsilon, i.e., the difference between 1.0 and the next value, representable as an \'unsigned long long int\' value;" << endl;
    cout << "\t-) has " << (unsigned long long int)std::numeric_limits<unsigned long long int>::round_error() << " as the maximum rounding error;" << endl;
    cout << "\t-) has " << (unsigned long long int)std::numeric_limits<unsigned long long int>::denorm_min() << " as the minimum positive subnormal \'unsigned long long int\' value;" << endl;
    ...
    switch (std::numeric_limits<unsigned long long int>::round_style)
    {
        case round_toward_zero:
            ...
            cout << "\t-) exploits the \'round toward zero\' policy for rounding not completely \'unsigned long long int\' values." << endl << endl;
            cout.flush();
            break;
            
        case round_to_nearest:
            
            cout << "\t-) exploits the \'round to nearest\' policy for rounding not completely \'unsigned long long int\' values." << endl << endl;
            cout.flush();
            break;

        case round_toward_infinity:
            
            cout << "\t-) exploits the \'round to the positive infinity\' policy for rounding not completely \'unsigned long long int\' values." << endl << endl;
            cout.flush();
            break;
            
        case round_toward_neg_infinity:
            
            cout << "\t-) exploits the \'round to the negative infinity\' policy for rounding not completely \'unsigned long long int\' values." << endl << endl;
            cout.flush();
            break;
            
        default:
            
            cout << "\t-) does not exploit a policy for rounding not completely \'unsigned long long int\' values." << endl << endl;
            cout.flush();
            break;
    }
    
    /* TASK #3 - finalizing our analysis! */
    cout << "\tIn the current platform, the maximum value for an object of \'unsigned long long int\' type is " << ULLONG_MAX << " (see the ULLONG_MAX macro)." << endl << endl;
